...
def make_cfg(settings):
    ...
    def create_tdv_camera_spec(**kw_args):
        camera_sensor_spec = habitat_sim.CameraSensorSpec()
        camera_sensor_spec.sensor_type = habitat_sim.SensorType.COLOR
        camera_sensor_spec.resolution = [settings["tdv_height"], settings["tdv_width"]]
        camera_sensor_spec.position = [0, 1.0, 0]
        camera_sensor_spec.orientation = [-np.pi/2., 0, 0]
        for k in kw_args:
            if k != "hfov":
    ...
    if settings["ortho_rgba_sensor"]:
        ortho_rgba_sensor_spec = create_tdv_camera_spec(
    ...
    if settings["ortho_depth_sensor"]:
        ortho_depth_sensor_spec = create_tdv_camera_spec(
    ...
    if settings["ortho_semantic_sensor"]:
        ortho_semantic_sensor_spec = create_tdv_camera_spec(
    ...
    if settings["multiview"]:
        multiview_orientation = {"front": 0, "left": 90 / 180 * np.pi,
                                 "right": - 90 / 180 * np.pi, "back": np.pi}
        for name, orient in multiview_orientation.items():
            color_sensor_spec = create_camera_spec(
                uuid="color_{}_sensor".format(name),
                hfov=settings["hfov"],
                sensor_type=habitat_sim.SensorType.COLOR,
                sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
            ...
            depth_sensor_spec = create_camera_spec(
                uuid="depth_{}_sensor".format(name),
                hfov=settings["hfov"],
                sensor_type=habitat_sim.SensorType.DEPTH,
                channels=1,
                sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
            ...
            semantic_sensor_spec = create_camera_spec(
                uuid="semantic_{}_sensor".format(name),
                hfov=settings["hfov"],
                sensor_type=habitat_sim.SensorType.SEMANTIC,
                channels=1,
                sensor_subtype=habitat_sim.SensorSubType.PINHOLE,
    ...
    agent_cfg.action_space = {
        ...
        #     "stop", habitat_sim.agent.ActuationSpec(amount=0)
        # ),
